...
M.get_node_modules_dir = function(opts)
  if vim.bo.node_modules_dir == nil then
    ...
    vim.bo.node_modules_dir = res[1] or false
  end
  return vim.bo.node_modules_dir
end

M.get_eslint = function()
  if vim.bo.eslint == nil then
    local nmd = M.get_node_modules_dir()
    if nmd == false then
      return false
    end
    local eslint_path = vim.fs.joinpath(nmd, ".bin/eslint")
    vim.bo.eslint = require("dko.utils.file").find_exists({ eslint_path })
      or false
  end
  return vim.bo.eslint
end
