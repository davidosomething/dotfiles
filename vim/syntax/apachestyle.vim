...
" WNO: Changed the "*" at the end of this pattern to \+ so that
" comments would work properly.
" syn match  apOption	/^\s*[0-9A-Za-z_]*/
" 
syn match  apOption	/^\s*[0-9A-Za-z_]\+/
...
" WNO: Removed (perhaps temporarily) since it flagged <<include...>> as
" an error.
" 
"syn match  apTagError	contained /[^>]</ms=s+1

" WNO: An attempt to highlight <<include ...>>
"
syn region apTagInclude start=/<<\s*include/ end=/>>/ contains=apTagOption,apTagError

" WNO: Make %MACROS% and $VARIABLES stand out.
" 
syn match apVariable /$[A-Za-z0-9_]\+/
...
if version >= 508 || !exists("did_apachestyle_syn_inits")
  ...
  HiLink apTagInclude   Include
  HiLink apMacro        Define
  HiLink apVariable     Identifier
